...
| Runtime port (`parse.l` + `case.l` + `com.l`) | Working; ~180 functions (buffer, nodes, case frames, the parse loop, the `stree`/`ctree` tree printers); drives 51 end-to-end integration parses | `system/core/runtime/` |
...
`test/integration/` (51 of them, all passing): a real sentence is tokenized,
